...
@onready var save_button: Button = $CanvasLayer/SaveButton
...
func _ready() -> void:
	...
	# Connect save button
	if save_button:
		save_button.pressed.connect(_on_save_values)
...
func _on_save_values() -> void:
	if not weapon_data:
		_show_feedback("ERROR: No weapon loaded!", Color.RED)
		return

	# Update the weapon data with current values
	weapon_data.fps_mesh_position = edit_position
	weapon_data.fps_mesh_rotation = edit_rotation
	weapon_data.fps_mesh_scale = edit_scale

	# Get the resource path to save to
	var save_path: String = weapon_data.resource_path
	if save_path.is_empty():
		_show_feedback("ERROR: No save path!", Color.RED)
		return

	# Save the resource
	var error: Error = ResourceSaver.save(weapon_data, save_path)
	if error == OK:
		print("[ViewmodelEditor] SAVED to: %s" % save_path)
		...
		print("[ViewmodelEditor] ERROR saving: %s (error %d)" % [save_path, error])
		_show_feedback("SAVE FAILED!\nError: %d" % error, Color.RED)


func _show_feedback(text: String, color: Color = Color.GREEN) -> void:
	copy_feedback_timer = 2.5
	if copy_feedback:
		copy_feedback.text = text
		copy_feedback.add_theme_color_override("font_color", color)
		copy_feedback.visible = true
...
OTHER:
  ...
  C - Copy to clipboard
  Ctrl+S - SAVE to .tres file
  F5 - Reload model

Changes are saved directly to
the weapon's .tres file!"""
