...
## THE LIVING WORLD
##
## An NPC placed here is a body. This section gives them a day: an archetype
## naming a file in data/schedules/archetypes/, and up to three stations - home,
## work, leisure - which NPCScheduler moves them between hour by hour.
##
## All 112 records in data/npc_schedules.json were generated by a Python script
## from a headless census, because there was no way to author one by hand.
## There is now.
var schedule_section: VBoxContainer
var schedule_archetype_option: OptionButton
var schedule_stations_label: RichTextLabel
var schedule_interior_check: CheckBox
var schedule_sit_check: CheckBox
var schedule_archetypes: Array[Dictionary] = []  # {id, description, blocks}

## Day preview: the hour scrub over the whole town.
var day_preview_check: CheckButton
var day_preview_slider: HSlider
var day_preview_label: Label
var day_preview_hour: int = 9
var day_preview_active: bool = false
var _authored_positions: Dictionary = {}  # element id -> Vector3, restored on exit
...
func _build_toolbar(parent: Control) -> void:
	...
	var schedules_btn := Button.new()
	schedules_btn.text = "Write schedules"
	schedules_btn.tooltip_text = "Merge every scheduled NPC in this layout into data/npc_schedules.json, converting their stations into the absolute world coordinates NPCScheduler reads. Records belonging to other towns are untouched."
	schedules_btn.pressed.connect(_write_schedules)
	toolbar.add_child(schedules_btn)

	toolbar.add_child(VSeparator.new())

	_build_day_preview(toolbar)
...
func _build_inspector(parent: Control) -> void:
	...
	# Their day
	_build_schedule_section(npc_section)
...
## ============================================================================
## THE LIVING WORLD - archetype, stations, and the hour scrub
## ============================================================================

const ARCHETYPE_DIR := "res://data/schedules/archetypes"
const SCHEDULE_RECORDS_PATH := "res://data/npc_schedules.json"
const STATION_KEYS: Array[String] = ["home", "work", "leisure"]
## NPCScheduler's own list. An action outside it fails validate_content.
const SCHEDULE_ACTIONS: Array[String] = ["sleep", "travel", "work", "eat", "socialise", "idle"]
## validate_content checks a quest giver is present and awake across these hours.
const DAY_FIRST_HOUR := 9
const DAY_LAST_HOUR := 17


func _build_schedule_section(parent: VBoxContainer) -> void:
	_load_schedule_archetypes()

	schedule_section = VBoxContainer.new()
	schedule_section.visible = false
	schedule_section.add_theme_constant_override("separation", 4)
	parent.add_child(schedule_section)

	schedule_section.add_child(HSeparator.new())

	var header := _make_label("Their day")
	header.add_theme_font_size_override("font_size", 12)
	header.add_theme_color_override("font_color", Color(0.65, 0.75, 0.95))
	schedule_section.add_child(header)

	var help := Label.new()
	help.text = "Pick a trade, then stand this NPC where they sleep, work and drink, pressing the matching button at each spot."
	help.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	help.add_theme_font_size_override("font_size", 10)
	help.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
	schedule_section.add_child(help)

	var arch_row := HBoxContainer.new()
	schedule_section.add_child(arch_row)
	var arch_label := _make_label("Trade:")
	arch_label.custom_minimum_size.x = 90
	arch_row.add_child(arch_label)
	schedule_archetype_option = OptionButton.new()
	schedule_archetype_option.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	schedule_archetype_option.add_item("-- none, no schedule --")
	schedule_archetype_option.set_item_metadata(0, "")
	for arch: Dictionary in schedule_archetypes:
		schedule_archetype_option.add_item(String(arch["id"]).capitalize())
		var idx: int = schedule_archetype_option.item_count - 1
		schedule_archetype_option.set_item_metadata(idx, arch["id"])
		schedule_archetype_option.set_item_tooltip(idx, String(arch.get("description", "")))
	schedule_archetype_option.item_selected.connect(_on_schedule_archetype_selected)
	arch_row.add_child(schedule_archetype_option)

	var flags_row := HBoxContainer.new()
	schedule_section.add_child(flags_row)
	schedule_interior_check = CheckBox.new()
	schedule_interior_check.text = "indoors (absent)"
	schedule_interior_check.tooltip_text = "The room behind that door is not modelled, so the NPC is simply not in the world while the schedule has them there. Do not tick this for a station you built a room for."
	flags_row.add_child(schedule_interior_check)
	schedule_sit_check = CheckBox.new()
	schedule_sit_check.text = "sitting"
	schedule_sit_check.tooltip_text = "They sit at this station - a bench, a bar stool, a pew."
	flags_row.add_child(schedule_sit_check)

	var buttons_row := HBoxContainer.new()
	buttons_row.add_theme_constant_override("separation", 3)
	schedule_section.add_child(buttons_row)
	for key: String in STATION_KEYS:
		var btn := Button.new()
		btn.text = "Set %s here" % key
		btn.tooltip_text = "Record where this NPC is standing right now as their %s station, with the two checkboxes above and their current facing." % key
		btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		btn.pressed.connect(_on_set_station_pressed.bind(key))
		buttons_row.add_child(btn)

	schedule_stations_label = RichTextLabel.new()
	schedule_stations_label.bbcode_enabled = true
	schedule_stations_label.fit_content = true
	schedule_stations_label.custom_minimum_size.y = 60
	schedule_stations_label.add_theme_font_size_override("normal_font_size", 10)
	schedule_section.add_child(schedule_stations_label)

	var clear_btn := Button.new()
	clear_btn.text = "Forget this NPC's day"
	clear_btn.pressed.connect(_on_clear_schedule_pressed)
	schedule_section.add_child(clear_btn)


## Read the trades off disk, so the dropdown can never offer an archetype that
## does not exist - which is the first thing validate_content checks.
func _load_schedule_archetypes() -> void:
	schedule_archetypes.clear()
	var dir := DirAccess.open(ARCHETYPE_DIR)
	if dir == null:
		push_warning("[TownEditor] No archetype directory at %s" % ARCHETYPE_DIR)
		return
	...
	for file_name: String in names:
		if not file_name.ends_with(".json"):
			continue
		var text: String = FileAccess.get_file_as_string(ARCHETYPE_DIR.path_join(file_name))
		var parsed: Variant = JSON.parse_string(text)
		if not (parsed is Dictionary):
			continue
		var data: Dictionary = parsed
		schedule_archetypes.append({
			"id": String(data.get("id", file_name.get_basename())),
			"description": String(data.get("description", "")),
			"nocturnal": bool(data.get("nocturnal", false)),
...
## Which station an archetype wants at this hour, and what it is doing there.
func _block_at_hour(archetype_id: String, hour: int) -> Dictionary:
	var arch: Dictionary = _archetype_by_id(archetype_id)
	if arch.is_empty():
		return {"action": "idle", "station": "work"}
	for block: Dictionary in arch.get("blocks", []) as Array:
		var from_hour: int = int(block.get("from", 0))
		var to_hour: int = int(block.get("to", 24))
		var inside: bool = (hour >= from_hour and hour < to_hour) if from_hour <= to_hour \
			else (hour >= from_hour or hour < to_hour)
		if inside:
			return {"action": String(block.get("action", "idle")), "station": String(block.get("station", "work"))}
	return {"action": "idle", "station": "work"}


func _selected_element() -> LevelEditorData.PlacedElement:
	if selected_element_id.is_empty():
		return null
	for elem: LevelEditorData.PlacedElement in level_data.elements:
		if elem.id == selected_element_id:
...
func _schedule_of(elem: LevelEditorData.PlacedElement) -> Dictionary:
	var sched: Variant = elem.properties.get("schedule", null)
	if sched is Dictionary:
		return sched
	return {}


func _on_schedule_archetype_selected(index: int) -> void:
	var elem: LevelEditorData.PlacedElement = _selected_element()
	if elem == null:
		return
	var archetype: String = String(schedule_archetype_option.get_item_metadata(index))
	if archetype.is_empty():
		elem.properties.erase("schedule")
		_refresh_schedule_readout(elem)
		_set_status("No schedule: this NPC stands where they were placed, all day.")
		return

	var sched: Dictionary = _schedule_of(elem)
	if sched.is_empty():
		sched = {"archetype": archetype, "stations": {}}
	else:
		sched["archetype"] = archetype
	elem.properties["schedule"] = sched
	_refresh_schedule_readout(elem)
	var arch: Dictionary = _archetype_by_id(archetype)
	_set_status("%s: %s" % [archetype.capitalize(), arch.get("description", "")])


## Record where the NPC is standing as one of their three stations.
##
## Stored LOCAL to the scene, exactly like every other element position, so the
## layout stays portable. The conversion to the absolute world coordinates
## NPCScheduler wants happens once, at write time, in _write_schedules().
func _on_set_station_pressed(station_key: String) -> void:
	var elem: LevelEditorData.PlacedElement = _selected_element()
	if elem == null:
		_set_status("Select an NPC first")
		return
	if elem.element_type != LevelEditorData.ElementType.NPC:
		_set_status("Only an NPC has a day")
		return

	var sched: Dictionary = _schedule_of(elem)
	if sched.is_empty() or String(sched.get("archetype", "")).is_empty():
		_set_status("Pick a trade first - the trade is what decides when they are at each station.")
		return

	var stations: Dictionary = sched.get("stations", {})
	stations[station_key] = {
		"local": [elem.position.x, elem.position.y, elem.position.z],
		"facing": int(round(elem.rotation.y)),
		"interior": schedule_interior_check.button_pressed,
		"sit": schedule_sit_check.button_pressed,
	}
	sched["stations"] = stations
	elem.properties["schedule"] = sched

	_refresh_schedule_readout(elem)
	_set_status("%s station set at (%.1f, %.1f)" % [station_key, elem.position.x, elem.position.z])


func _on_clear_schedule_pressed() -> void:
	var elem: LevelEditorData.PlacedElement = _selected_element()
	if elem == null:
		return
	elem.properties.erase("schedule")
	schedule_archetype_option.select(0)
	_refresh_schedule_readout(elem)
	_set_status("Schedule cleared")


## Say what is set, what is missing, and what the game will refuse.
func _refresh_schedule_readout(elem: LevelEditorData.PlacedElement) -> void:
	if schedule_stations_label == null:
		return
	var sched: Dictionary = _schedule_of(elem)
	if sched.is_empty():
		schedule_stations_label.text = "[color=#888]No day. This NPC stands where you put them, at every hour.[/color]"
		return

	var archetype: String = String(sched.get("archetype", ""))
	var stations: Dictionary = sched.get("stations", {})
	var lines: Array[String] = []

	for key: String in STATION_KEYS:
		if stations.has(key):
			var st: Dictionary = stations[key]
			var local: Array = st.get("local", [0, 0, 0])
			var notes: Array[String] = []
			if st.get("interior", false):
				notes.append("indoors")
			if st.get("sit", false):
				notes.append("sitting")
			var suffix: String = "  (%s)" % ", ".join(notes) if not notes.is_empty() else ""
			lines.append("[color=#8fbf8f]%s[/color]  %.0f, %.0f%s" % [key, float(local[0]), float(local[2]), suffix])
		else:
			lines.append("[color=#666]%s  - not set[/color]" % key)

	# The rules validate_content will apply, said here instead of at commit time.
	if not stations.has("work"):
		lines.append("[color=#e08060]Every record must have a work station. This one will not be written.[/color]")

	var quest_ids: Array = elem.properties.get("quest_ids", [])
	if not quest_ids.is_empty() and not archetype.is_empty():
		var reachable: bool = false
		for hour: int in range(DAY_FIRST_HOUR, DAY_LAST_HOUR + 1):
			var block: Dictionary = _block_at_hour(archetype, hour)
			if block["action"] == "sleep":
				continue
			var wanted: String = block["station"]
			var st2: Dictionary = stations.get(wanted, stations.get("work", {}))
			if not st2.is_empty() and not st2.get("interior", false):
				reachable = true
				break
		if not reachable:
			lines.append("[color=#e08060]This NPC gives a quest but cannot be reached between %02d:00 and %02d:00.[/color]" % [DAY_FIRST_HOUR, DAY_LAST_HOUR])

	schedule_stations_label.text = "\n".join(lines)
...
func _build_day_preview(parent: HBoxContainer) -> void:
	day_preview_check = CheckButton.new()
	day_preview_check.text = "Day preview"
	day_preview_check.tooltip_text = "Move every scheduled NPC to where they stand at the chosen hour. Nothing is saved; untick to put everyone back where you placed them."
	day_preview_check.toggled.connect(_on_day_preview_toggled)
	parent.add_child(day_preview_check)

	day_preview_slider = HSlider.new()
	day_preview_slider.min_value = 0
	day_preview_slider.max_value = 23
	day_preview_slider.step = 1
	day_preview_slider.value = day_preview_hour
	day_preview_slider.custom_minimum_size.x = 150
	day_preview_slider.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	day_preview_slider.value_changed.connect(_on_day_preview_hour_changed)
	parent.add_child(day_preview_slider)

	day_preview_label = Label.new()
	day_preview_label.custom_minimum_size.x = 260
	day_preview_label.text = "09:00"
	parent.add_child(day_preview_label)


func _on_day_preview_toggled(pressed: bool) -> void:
	day_preview_active = pressed
	if pressed:
		_authored_positions.clear()
		for elem: LevelEditorData.PlacedElement in level_data.elements:
			_authored_positions[elem.id] = elem.position
		_apply_day_preview()
	else:
		for elem_id: String in _authored_positions:
			var node: Node3D = placed_elements.get(elem_id)
			if is_instance_valid(node):
				node.position = _authored_positions[elem_id]
		_authored_positions.clear()
		day_preview_label.text = "%02d:00" % day_preview_hour
		_set_status("Everyone is back where you placed them")


func _on_day_preview_hour_changed(value: float) -> void:
	day_preview_hour = int(value)
	if day_preview_active:
		_apply_day_preview()
	else:
		day_preview_label.text = "%02d:00" % day_preview_hour


## Stand the town up at one hour. This is the ship gate for a living town, read
## with your eyes: a town whose 03:00 looks like its 13:00 has a clock, not a
## life. (tools/check_living_world.gd says the same thing in assertions.)
func _apply_day_preview() -> void:
	var absent: int = 0
	var placed: int = 0
	var unscheduled: int = 0
	var actions: Dictionary = {}

	for elem: LevelEditorData.PlacedElement in level_data.elements:
		if elem.element_type != LevelEditorData.ElementType.NPC:
			continue
		var node: Node3D = placed_elements.get(elem.id)
		...
		var block: Dictionary = _block_at_hour(archetype, day_preview_hour)
		var stations: Dictionary = sched.get("stations", {})
		var station: Dictionary = stations.get(String(block["station"]), stations.get("work", {}))

		if station.is_empty() or station.get("interior", false):
			# Absent means absent. NPCScheduler hides them; so does the preview.
			node.visible = false
			absent += 1
			continue

		var local: Array = station.get("local", [0, 0, 0])
		node.visible = true
		node.position = Vector3(float(local[0]), float(local[1]), float(local[2]))
		node.rotation_degrees.y = float(station.get("facing", 0))
		placed += 1
		var action: String = String(block["action"])
		actions[action] = int(actions.get(action, 0)) + 1

	var parts: Array[String] = []
	for action: String in actions:
		parts.append("%d %s" % [actions[action], action])
	parts.sort()
	day_preview_label.text = "%02d:00  -  %d out, %d indoors%s%s" % [
		day_preview_hour, placed, absent,
		("  (" + ", ".join(parts) + ")") if not parts.is_empty() else "",
		("  |  %d unscheduled" % unscheduled) if unscheduled > 0 else ""
	]


## ---------------------------------------------------------------------------
## Writing data/npc_schedules.json
## ---------------------------------------------------------------------------

## Merge this town's scheduled NPCs into the records file.
##
## THE CONVERSION, and the only place it happens: a station's `pos` is absolute
## world space - `WorldGrid.cell_to_world(cell) + local` - because that is what
## NPCScheduler reads and what validate_content measures. Element positions are
## scene-local. Getting this backwards puts every station a few hundred metres
## from the cell it claims, and the validator says so.
##
## Only ids this layout carries are touched. Every other record in the file is
## left exactly as it was, because 112 of them belong to towns that are not open.
func _write_schedules() -> void:
	var cell := Vector2i(
		int(level_data.metadata.get("world_x", 0)),
		int(level_data.metadata.get("world_y", 0))
	)
	var zone: String = level_data.level_id if not level_data.level_id.is_empty() \
		else level_data.level_name.to_snake_case()
	...
	if world_grid_script == null:
		...
		return
	var cell_origin: Vector3 = world_grid_script.cell_to_world(cell)

	var doc: Dictionary = {}
	if FileAccess.file_exists(SCHEDULE_RECORDS_PATH):
		var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(SCHEDULE_RECORDS_PATH))
		if parsed is Dictionary:
			doc = parsed
	if not doc.has("npcs"):
		doc["npcs"] = {}
	var records: Dictionary = doc["npcs"]
	...
	for elem: LevelEditorData.PlacedElement in level_data.elements:
		...
		var npc_id: String = String(elem.properties.get("npc_id", ""))
		if npc_id.is_empty():
			skipped.append("%s (no npc_id)" % elem.properties.get("name", elem.id))
			continue

		var stations: Dictionary = sched.get("stations", {})
		if not stations.has("work"):
			skipped.append("%s (no work station)" % npc_id)
			continue

		var out_stations: Dictionary = {}
		for key: String in STATION_KEYS:
			...
			var world_pos := Vector3(
				cell_origin.x + float(local[0]),
				float(local[1]),
				cell_origin.z + float(local[2])
			)
			var entry: Dictionary = {
				"cell": [cell.x, cell.y],
				"pos": [snappedf(world_pos.x, 0.01), snappedf(world_pos.y, 0.01), snappedf(world_pos.z, 0.01)],
				"facing": int(st.get("facing", 0)),
			}
			if st.get("interior", false):
				entry["interior"] = true
			if st.get("sit", false):
				entry["sit"] = true
			out_stations[key] = entry
		...
		if not (elem.properties.get("quest_ids", []) as Array).is_empty():
			record["quest_giver"] = true
	...
	var file := FileAccess.open(SCHEDULE_RECORDS_PATH, FileAccess.WRITE)
	if not file:
		_set_status("ERROR: could not write %s" % SCHEDULE_RECORDS_PATH)
		return
	file.store_string(JSON.stringify(doc, "\t"))
	file.close()

	var msg: String = "Wrote %d schedules into %s (cell %d,%d)" % [written, SCHEDULE_RECORDS_PATH.get_file(), cell.x, cell.y]
	if not skipped.is_empty():
		msg += "  -  skipped: %s" % ", ".join(skipped)
	_set_status(msg)
	print("[TownEditor] %s" % msg)
...
func _refresh_inspector_for_selection(elem: Variant) -> void:
	...
	if is_npc:
		...
		# Their day - every NPC has one, or explicitly has none.
		if schedule_section:
			schedule_section.visible = true
			var sched: Dictionary = _schedule_of(elem)
			var current_archetype: String = String(sched.get("archetype", ""))
			for i: int in range(schedule_archetype_option.item_count):
				if String(schedule_archetype_option.get_item_metadata(i)) == current_archetype:
					schedule_archetype_option.select(i)
					break
			_refresh_schedule_readout(elem)
	else:
		merchant_section.visible = false
