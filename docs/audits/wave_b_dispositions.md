...
| G-2 | **The map says Kazan-Dun; everything else says Kazan-Dun.** `world_grid.gd` has `kazan_dun_entrance`, "Kazan-Dun South Gate" and "Road to Kazan-Dun"; the bible, the flags (`kazan_dun_helped`), the quest folder and every dialogue file say Kazan. | A rename is text-only on one side and an id change on the other. Which spelling is the hold's actual name is his |
...
## 8/2 (later) — the editor tool suite

### Opened by this pass — these need Caleb

| # | Question | Why it is blocked |
|---|---|---|
| T-1 | **Twelve places are declared at coordinates the grid does not contain**: `falkenhaften`, `bandit_camp_tundra_west`, `bandit_camp_tundra_east`, `goblin_camp_tundra`, `ruined_temple_frost`, `bandit_camp_eastern_road`, `goblin_camp_eastern_hills`, `ruined_temple_eastern`, `bandit_camp_desert`, `smuggler_cove`, `pirate_camp_island`, `ruined_temple_island`. They are rows in `world_grid.gd`'s own `LOCATIONS`, outside `GRID_MIN..GRID_MAX`, so `initialize()` builds no cell for them and **`get_location_coords()` answers `(0, 0)`, which is Elder Moor.** Anything that asks where Falkenhaften is is told it is the starting town. | Where each belongs is a design decision and not a merge. Either the grid grows north, south and east to hold them - which is what the legacy 64x64 forge map assumed - or each row moves inside the present bounds. Recorded as a ceiling in `tools/check_editor_tools.gd`'s `OUT_OF_BOUNDS_LOCATIONS`, which fails if a thirteenth appears |
