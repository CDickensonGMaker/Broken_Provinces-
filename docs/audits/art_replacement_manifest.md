...
|---|---|---|---|
| `assets/sprites/legacy/npcs/temple/` (no female sheet) | The temple set is three male monks (`monk_1/2/3_purple`). Sister Edith and every priestess the game names have nowhere to stand. Repointed 8/2 to `civilians/lady_in_red.png` (8x1), which is at least a woman, and is the same fallback QuestGiver uses | `data/npcs/sister_edith.tres`, every `Priestess of` spawn | One female monk/robed sheet at 8x1 matching the civilian frame rate |
| `assets/sprites/legacy/npcs/dwarves/` (no female sheet) | Same shape: the dwarf set has no woman in it, so a dwarf woman falls back to a human civilian | `civilian_npc.gd` dwarf spawns | A female dwarf sheet |
| `assets/sprites/legacy/npcs/civilians/man_civilian.png` | Declared as **1 frame** in `quest_giver.gd` and the zoo, and as **8x2** in `merchant.gd` and `townsfolk.gd`. One of those is wrong and has been since both were written; the sheet is 48x96, i.e. one frame, so every caller passing 8x2 is slicing a single pose into sixteen | `quest_giver.gd`, `merchant.gd`, `townsfolk.gd`, `zoo_registry.gd` | Caleb's call: either a real 8-frame man sheet, or the 8x2 call sites corrected to 1x1 |
