...
**Phase 0 — BUILT, 2026-08-02.** `CharacterVisual` (`scripts/characters/visuals/character_visual.gd`) is one interface behind CivilianNPC (`scripts/characters/npcs/civilian_npc.gd:_create_visual`) and QuestGiver (`scripts/characters/npcs/quest_giver.gd:_create_visual`), which between them are every named and ambient townsperson: `set_action` · `set_moving` · `set_facing` · `play_attack/hurt/death` · `set_talking` · `set_tint`. Two implementations — `BillboardVisual` (wraps today's BillboardSprite; the default and a measured no-op) and `RiggedVisual` (a citizen GLB, dressed at runtime). `NPCScheduler.place_for_hour` hands the living-world action to whichever body is there. The flip is ONE data field: `data/character_models.json` maps npc_id → master, absent = billboard, no code change per character. Gate: `tools/check_character_visual.tscn`.
