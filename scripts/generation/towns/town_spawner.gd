	...
	match archetype:
		...
		"priest":
			npc = CivilianNPC.spawn_civilian(
				self,
				pos,
				sprite_path if not sprite_path.is_empty() else "res://assets/sprites/legacy/npcs/civilians/monk_tan.png",
				h_frames,
				v_frames,
				false,
				0.0384
...
func _spawn_merchant(pos: Vector3, npc_name: String, shop_type: String, shop_tier: int, actor_data: Dictionary) -> Merchant:
	...
	var sprite_path: String = actor_data.get("sprite_path", "res://assets/sprites/legacy/npcs/merchants/merchant_civilian.png")
