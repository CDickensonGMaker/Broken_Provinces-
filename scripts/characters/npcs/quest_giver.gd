...
func _create_visual() -> void:
	...
	if not tex:
		if is_female:
			...
			tex = load("res://assets/sprites/legacy/npcs/civilians/lady_in_red.png") as Texture2D
			h_frames = 8  # lady_in_red is 8-frame sheet
			v_frames = 1
			pixel_size = 0.0256  # 96px frame, 2.46m target
		else:
			# Use man_civilian sprite for male NPCs
			tex = load("res://assets/sprites/legacy/npcs/civilians/man_civilian.png") as Texture2D
