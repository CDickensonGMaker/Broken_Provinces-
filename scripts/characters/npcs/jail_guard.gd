...
const GUARD_SPRITE := "res://assets/sprites/legacy/npcs/civilians/guard_civilian.png"
...
func take_damage(amount: int, damage_type: Enums.DamageType = Enums.DamageType.PHYSICAL, attacker: Node = null) -> int:
	...
	var armor_mult: float = 1.0 if CombatManager.is_armor_already_applied(self) else 100.0 / (100.0 + float(ARMOR))
...
static func spawn_jail_guard(parent: Node, pos: Vector3, p_prison: Node3D = null, p_region_id: String = "elder_moor") -> Node3D:
	var script: Script = load("res://scripts/characters/npcs/jail_guard.gd")
