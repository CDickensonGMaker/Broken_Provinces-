## bandit_hideout_level_1.gd - Bandit Hideout Level 1 (Hand-crafted)
## Cave tunnels with storage rooms and barracks
## All geometry, enemies, chests, and doors are pre-placed in the .tscn file
extends Node3D

const ZONE_ID := "bandit_hideout_level_1"
...
func _setup_spawn_points() -> void:
	...
	# Expected structure: SpawnPoints/SpawnPoint_Default, SpawnPoints/SpawnPoint_FromExterior, etc.
...
func _spawn_enemy_at_marker(marker: Node3D) -> void:
	...
	if not sprite_config.is_empty():
		...
		sprite_path = marker.get_meta("sprite_path", "res://assets/sprites/legacy/enemies/humanoid/human_bandit_alt.png")
		h_frames = marker.get_meta("h_frames", 4)
		v_frames = marker.get_meta("v_frames", 1)

	var sprite_texture: Texture2D = load(sprite_path)
	if not sprite_texture:
		push_error("[BanditHideoutL1] Failed to load sprite: %s" % sprite_path)
		return
...
func _spawn_door_at_marker(marker: Node3D, parent: Node3D) -> void:
	...
	if target_scene.is_empty():
		push_warning("[BanditHideoutL1] Door marker %s has no target_scene" % marker.name)
		return
