## bandit_camp_east.gd - Eastern Bandit Outpost
## An eastern outpost where bandits watch the road to Thornfield
## Located at grid coords (4, -1)
extends Node3D

const ZONE_ID := "bandit_camp_east"
...
func _setup_environment() -> void:
	...
	env.background_color = Color(0.22, 0.28, 0.2)
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	env.ambient_light_color = Color(0.35, 0.40, 0.30)
	env.ambient_light_energy = 0.5
	env.fog_enabled = true
	env.fog_light_color = Color(0.25, 0.30, 0.22)
	...
	light.light_color = Color(0.88, 0.85, 0.75)
...
func _spawn_enemies_from_markers() -> void:
	...
	if not enemy_spawns:
		push_warning("[BanditCampEast] EnemySpawns node not found")
		return
	...
	for marker in enemy_spawns.get_children():
		if marker is Marker3D:
			...
			if not sprite_texture:
				push_error("[BanditCampEast] Failed to load sprite: %s" % sprite_path)
				continue
