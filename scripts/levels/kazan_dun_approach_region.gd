## kazan_dun_approach_region.gd - The Kazan-Dun Approach region (mountain foothills)
## Rocky terrain leading to the Dwarf stronghold of Kazan-Dun
extends Node3D

const ZONE_ID := "region_kazan_dun_approach"
...
func _setup_navigation() -> void:
	if not nav_region:
		push_warning("[Kazan-Dun Approach] NavigationRegion3D not found")
		return
...
func _spawn_enemies() -> void:
	# Kazan-Dun Approach has harder enemies - orcs, goblins, mountain beasts
...
func _spawn_enemy_at_marker(marker: Marker3D) -> void:
	var enemy_data_path: String = marker.get_meta("enemy_data", "res://data/enemies/goblin_soldier.tres")
	var enemy_type: String = marker.get_meta("enemy_type", "goblin_soldier")

	# Default values from marker metadata
	var sprite_path: String = marker.get_meta("sprite_path", "res://assets/sprites/legacy/enemies/goblins/goblin_sword.png")
	...
	if not sprite_texture:
		push_error("[Kazan-Dun Approach] Failed to load sprite: %s" % sprite_path)
		return
...
func _spawn_herbs() -> void:
	# Sparse mountain herbs - harder to find but more valuable
