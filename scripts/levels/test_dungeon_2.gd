...
func _ready() -> void:
	...
	SaveManager.set_current_zone(ZONE_ID, "Forgotten Tombs")
...
func _initialize_seed() -> void:
	...
	print("[ForgottenTombs] Using seed: %d" % dungeon_seed)
...
## Create ambient lighting and environment - GREEN THEME
func _create_environment() -> void:
	# World environment
	var env := Environment.new()
	env.background_mode = Environment.BG_COLOR
	env.background_color = Color(0.02, 0.04, 0.02)  # Greenish black
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	env.ambient_light_color = Color(0.1, 0.18, 0.12)  # Green tint
	env.ambient_light_energy = 0.4
	env.fog_enabled = true
	env.fog_light_color = Color(0.06, 0.12, 0.08)  # Green fog
	...
	# Directional light (dim, greenish)
	var light := DirectionalLight3D.new()
	light.light_color = Color(0.5, 0.7, 0.5)  # Green tint
...
func _create_entrance_template() -> void:
	...
	template.floor_color = Color(0.08, 0.12, 0.08)
	template.wall_color = Color(0.12, 0.18, 0.1)
	template.ceiling_color = Color(0.1, 0.14, 0.08)
...
func _create_corridor_template() -> void:
	...
	template.floor_color = Color(0.06, 0.1, 0.06)
	template.wall_color = Color(0.1, 0.15, 0.08)
...
func _create_empty_room_template() -> void:
	...
	template.floor_color = Color(0.07, 0.11, 0.07)
	template.wall_color = Color(0.11, 0.16, 0.09)
...
func _create_treasure_room_template() -> void:
	...
	template.floor_color = Color(0.1, 0.14, 0.1)
	template.wall_color = Color(0.14, 0.2, 0.12)
...
func _create_shrine_template() -> void:
	...
	template.rest_spot_name = "Overgrown Shrine"
	...
	# Lighter green colors (sacred place)
	template.floor_color = Color(0.12, 0.16, 0.1)
	template.wall_color = Color(0.16, 0.22, 0.14)
...
func _create_quest_room_template() -> void:
	...
	template.quest_npc_name = "Tomb Guardian Spirit"
	template.quest_data_path = "res://data/quests/dungeon_clear.json"
	template.quest_npc_position = Vector3(0, 0, 2)

	# Ancient tomb decorations
	template.decorations = [
		RoomTemplate.make_decoration("pillar", Vector3(-3, 0, -3)),
		RoomTemplate.make_decoration("pillar", Vector3(3, 0, -3)),
	]

	# Mossy green safe room colors
	template.floor_color = Color(0.1, 0.14, 0.08)
	template.wall_color = Color(0.14, 0.2, 0.1)
	template.ceiling_color = Color(0.12, 0.16, 0.08)
...
func _create_boss_template() -> void:
	...
	# Deep green boss room
	template.floor_color = Color(0.06, 0.1, 0.06)
	template.wall_color = Color(0.1, 0.14, 0.08)
	template.ceiling_color = Color(0.08, 0.12, 0.06)
...
func _create_spawn_point() -> void:
	...
	if entrance:
		...
		print("[ForgottenTombs] Spawn point created at: %s" % spawn.position)
	else:
		# Fallback to origin with safety floor
		spawn.position = Vector3(0, 1.0, 0)
		_create_fallback_floor()
		push_warning("[ForgottenTombs] No entrance room found! Using fallback spawn at origin.")
...
func _create_fallback_floor() -> void:
	...
	mat.albedo_color = Color(0.1, 0.5, 0.1)  # Green to match theme
	floor_mesh.material = mat

	add_child(floor_mesh)
	print("[ForgottenTombs] Created fallback floor due to generation failure!")


func _on_generator_complete(_dungeon: DungeonGenerator) -> void:
	print("[ForgottenTombs] Dungeon generation complete!")
