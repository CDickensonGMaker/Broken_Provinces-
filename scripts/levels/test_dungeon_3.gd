## test_dungeon_3.gd - Shadow Depths procedural dungeon (production)
## NOTE: "test" in filename is historical - this is an active gameplay level
## Red-themed crypt dungeon
extends Node3D

const ZONE_ID := "shadow_depths"
...
func _ready() -> void:
	...
	SaveManager.set_current_zone(ZONE_ID, "Shadow Depths")
...
func _initialize_seed() -> void:
	...
	print("[ShadowDepths] Using seed: %d" % dungeon_seed)
...
## Create ambient lighting and environment - RED THEME
func _create_environment() -> void:
	# World environment
	var env := Environment.new()
	env.background_mode = Environment.BG_COLOR
	env.background_color = Color(0.04, 0.02, 0.02)  # Reddish black
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	env.ambient_light_color = Color(0.2, 0.1, 0.1)  # Red tint
	env.ambient_light_energy = 0.35
	env.fog_enabled = true
	env.fog_light_color = Color(0.12, 0.06, 0.06)  # Red fog
	...
	# Directional light (dim, reddish)
	var light := DirectionalLight3D.new()
	light.light_color = Color(0.8, 0.5, 0.4)  # Red/orange tint
...
func _setup_generator() -> void:
	...
	_create_empty_room_template()
	_create_treasure_room_template()
	_create_shrine_template()
	_create_quest_room_template()
...
func _create_entrance_template() -> void:
	...
	template.portal_spawn_id = "from_shadow_depths"
	template.portal_display_name = "Return to Town"

	# Blood-red stone colors
	template.floor_color = Color(0.12, 0.06, 0.06)
	template.wall_color = Color(0.18, 0.1, 0.1)
	template.ceiling_color = Color(0.14, 0.08, 0.08)
...
func _create_corridor_template() -> void:
	...
	template.floor_color = Color(0.1, 0.05, 0.05)
	template.wall_color = Color(0.15, 0.08, 0.08)
...
func _create_empty_room_template() -> void:
	...
	template.floor_color = Color(0.11, 0.055, 0.055)
	template.wall_color = Color(0.16, 0.09, 0.09)
...
func _create_treasure_room_template() -> void:
	...
	template.floor_color = Color(0.14, 0.07, 0.07)
	template.wall_color = Color(0.2, 0.11, 0.11)
...
func _create_shrine_template() -> void:
	...
	template.rest_spot_name = "Blood Altar"
	...
	# Slightly brighter red (ominous sacred place)
	template.floor_color = Color(0.16, 0.08, 0.08)
	template.wall_color = Color(0.22, 0.12, 0.12)
...
func _create_quest_room_template() -> void:
	...
	# No enemies - safe room for quest interaction
	template.min_enemies = 0
	template.max_enemies = 0

	# Quest NPC configuration
	template.has_quest_npc = true
	template.quest_npc_name = "Tormented Soul"
	...
	# Blood-red safe room colors (slightly warmer)
	template.floor_color = Color(0.14, 0.08, 0.06)
	template.wall_color = Color(0.2, 0.12, 0.1)
	template.ceiling_color = Color(0.16, 0.1, 0.08)
...
func _create_boss_template() -> void:
	...
	# Single entrance
	template.doors = [
		RoomTemplate.make_door(Vector3(0, 0, -8), Vector3.BACK, 5.0),
	]
	...
	# More minions (harder dungeon)
	template.min_enemies = 3
	template.max_enemies = 4
	template.enemy_spawn_zones = [
		Vector3(-5, 0, 0),
		Vector3(5, 0, 0),
	...
	# Boss loot (better rewards)
	template.chest_count = 2
	template.chest_locked = true
	template.chest_lock_dc = 16
	template.loot_tier = 3  # RARE
	template.loot_spawn_zones = [Vector3(-2, 0, 6), Vector3(2, 0, 6)]

	# Throne/altar for boss
	template.decorations = [
		RoomTemplate.make_decoration("altar", Vector3(0, 0, 6)),
		RoomTemplate.make_decoration("pillar", Vector3(-6, 0, -5)),
		RoomTemplate.make_decoration("pillar", Vector3(6, 0, -5)),
		RoomTemplate.make_decoration("pillar", Vector3(-6, 0, 5)),
		RoomTemplate.make_decoration("pillar", Vector3(6, 0, 5)),
		RoomTemplate.make_decoration("coffin", Vector3(-4, 0, 6), PI / 2),
		RoomTemplate.make_decoration("coffin", Vector3(4, 0, 6), -PI / 2),
	]

	# Deep blood red boss room
	template.floor_color = Color(0.1, 0.04, 0.04)
	template.wall_color = Color(0.14, 0.06, 0.06)
	template.ceiling_color = Color(0.12, 0.05, 0.05)
...
func _create_spawn_point() -> void:
	...
	if entrance:
		...
		print("[ShadowDepths] Spawn point created at: %s" % spawn.position)
	else:
		# Fallback to origin with safety floor
		spawn.position = Vector3(0, 1.0, 0)
		_create_fallback_floor()
		push_warning("[ShadowDepths] No entrance room found! Using fallback spawn at origin.")
...
func _create_fallback_floor() -> void:
	...
	mat.albedo_color = Color(0.5, 0.1, 0.1)  # Red to match theme
	floor_mesh.material = mat

	add_child(floor_mesh)
	print("[ShadowDepths] Created fallback floor due to generation failure!")


func _on_generator_complete(_dungeon: DungeonGenerator) -> void:
	print("[ShadowDepths] Dungeon generation complete!")
