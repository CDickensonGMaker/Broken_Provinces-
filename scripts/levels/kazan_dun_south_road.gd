## kazan_dun_south_road.gd - Southern Mountain Road from Kazan-Dun
## Large outdoor zone (120x120 units) - winding mountain path with switchbacks
## HUB zone connecting the dwarf hold south exit to southern wilderness
## Rocky terrain, scattered trees/shrubs, guard post near exit
extends Node3D

const ZONE_ID := "kazan_dun_south_road"
...
func _setup_navigation() -> void:
	if not nav_region:
		push_warning("[Kazan-Dun South Road] NavigationRegion3D not found in scene")
		return
...
## Spawn dwarf guards at the south gate
func _spawn_dwarf_npcs() -> void:
	...
	# Guards at the south gate (near entrance to the hold)
