## bandit_camp_south.gd - Southern Bandit Encampment
## A southern bandit camp that raids the road to Millbrook
## Located at grid coords (-1, 5)
extends Node3D

const ZONE_ID := "bandit_camp_south"
...
func _ready() -> void:
	...
	print("[BanditCampSouth] Camp loaded")


func _setup_environment() -> void:
	var env := Environment.new()
	env.background_mode = Environment.BG_COLOR
	env.background_color = Color(0.2, 0.26, 0.18)
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	env.ambient_light_color = Color(0.30, 0.36, 0.26)
	env.ambient_light_energy = 0.45
	env.fog_enabled = true
	env.fog_light_color = Color(0.20, 0.26, 0.18)
	...
	light.light_color = Color(0.82, 0.80, 0.70)
...
func _spawn_enemies_from_markers() -> void:
	...
	if not enemy_spawns:
		push_warning("[BanditCampSouth] EnemySpawns node not found")
		return
	...
	for marker in enemy_spawns.get_children():
		if marker is Marker3D:
			...
			if not sprite_texture:
				push_error("[BanditCampSouth] Failed to load sprite: %s" % sprite_path)
				continue
	...
	print("[BanditCampSouth] Spawned enemies from %d markers" % enemy_spawns.get_child_count())
...
func _setup_cell_streaming() -> void:
	...
	print("[BanditCampSouth] Streaming started at %s" % my_coords)
