## harvestable_tree.gd - Interactable tree that yields wood when harvested with an axe
...
## Tree textures - variety of tree sprites
const TREE_TEXTURES: Array[String] = [
	...
	"res://assets/sprites/legacy/environment/trees/green_sap_tree1.png",
	"res://assets/sprites/legacy/environment/trees/green_sap_tree2.png",
]

## Stump texture for chopped trees
const STUMP_TEXTURE: String = "res://assets/sprites/legacy/environment/trees/chopped_tree.png"
...
func _setup_interaction_area() -> void:
	...
	sphere.radius = 1.5  # Larger detection radius for trees
...
func _setup_visuals() -> void:
	...
	if tex:
		...
		sprite.position.y = approx_height / 2.0
...
func interact(_interactor: Node) -> void:
	...
	if InventoryManager.add_item("wood_plank", yield_amount):
		...
		var hud := _interactor.get_tree().get_first_node_in_group("hud") if _interactor else null
		if hud and hud.has_method("show_notification"):
			hud.show_notification("Chopped tree: %d Wood Planks" % yield_amount)

		# Visual feedback - hide the tree
		_on_harvested()
	else:
		# Inventory full
		var hud := _interactor.get_tree().get_first_node_in_group("hud") if _interactor else null
...
func _on_harvested() -> void:
	...
	# Change sprite to chopped stump
	if sprite:
		var stump_tex := load(STUMP_TEXTURE) as Texture2D
		if stump_tex:
			sprite.texture = stump_tex
			# Stump is shorter - reposition to ground level
			var stump_height: float = stump_tex.get_height() * sprite.pixel_size
			sprite.position.y = stump_height / 2.0
		else:
			# Fallback: hide if stump texture not found
...
## Static factory method for spawning trees
static func spawn_tree(parent: Node, pos: Vector3, p_display_name: String = "Tree") -> HarvestableTree:
	var instance := HarvestableTree.new()
